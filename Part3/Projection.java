...
public class Projection {
     ...
     */
    public static Point2D model2Screen(GisPoint modelPoint, NetworkViewer viewer) {
        return new Point2D(
                model2ScreenX(modelPoint.getLon(), viewer.getMapCanvas(), viewer.getScale(),
                        viewer.getRatioLatLon(), viewer.getOrigin().getLon()),
                model2ScreenY(modelPoint.getLat(), viewer.getMapCanvas(), viewer.getScale(),
                        viewer.getOrigin().getLat()));
    }

    /**
     * Project a point from the screen pixels to the Gis position
     * @param screenPoint canvas x and y
     * @param viewer contains the canvas, scale, and origin
     * @return a GisPoint corresponding to the displayed point
     */
    public static GisPoint screen2Model(Point2D screenPoint, NetworkViewer viewer) {
        return new GisPoint(
                getScreen2ModelX(screenPoint, viewer.getMapCanvas(), viewer.getScale(),
                        viewer.getRatioLatLon(), viewer.getOrigin().getLon()),
                getScreen2ModelY(screenPoint, viewer.getMapCanvas(), viewer.getScale(),
                        viewer.getOrigin().getLat()));
    }
